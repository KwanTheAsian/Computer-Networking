...
class DownloadFileWindow:
    def __init__(self):
        ...
        data_list = [["Filename"]]  # Header row
        data_list.extend([file] for file in listfiles)
...
def list_files():
    client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    client.settimeout(10)  # Thêm timeout 10 giây
    try:
        client.connect((HOST, SERVER_PORT))
        ...
        if response == "NO_FILES":
            ...
            while True:
                try:
                    chunk = client.recv(1024).decode(FORMAT).strip()
                    if chunk == "END":
                        break
                    if chunk:
